...
const TrustedBySection = ({ className }: TrustedBySectionProps) => {
  ...
  return (
    <div className={`flex flex-col items-center gap-6 py-4 bg-black text-white ${className}`}>
        ...
        <CarouselContent>
          {companies.map((company, index) => (
            <CarouselItem key={index} className="basis-1/3 md:basis-1/4 lg:basis-1/5 flex justify-center">
              <div className="text-secondary font-semibold text-center py-2">{company}</div>
            </CarouselItem>
          ))}
        </CarouselContent>
      </Carousel>
