...
export const useNewsletterExamples = () => {
  const { user } = useAuth();
  const queryClient = useQueryClient();

  // Fetch all newsletter examples (using documents table with content_type = 'newsletter')
  const fetchExamples = async (): Promise<NewsletterExample[]> => {
  ...
  // Add a new newsletter example
  const addNewExample = async (example: ExampleInput): Promise<NewsletterExample> => {
    if (!user) throw new Error('User not authenticated');
